...
function begin(){
    ...
    document.getElementById("question").innerHTML=[RN2[4]+operator[2]+RN1[3]];
...
function submit(){
    ...
    let correctanswer= Math.round(eval(question));//maybe I'll do this manually later
    //Let the eval function's answer be rounded to 2 d.p
    ...
    if (yn==0) {
        ...
        document.getElementById("question").innerHTML= parseInt(Math.random()*100) + "/" + parseInt(Math.random()*10);
...
function restart(){
    ...
    const RN2= [4,6,87,93,45,68,21,332,45,900]; //Random number 2
    const operator=["+","-","/","*","+","*","/","-","+","*"]; //Random operator
    document.getElementById("question").innerHTML=[RN2[9]+operator[2]+RN1[9]];
